...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "DoReMi-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name DoReMi-service apps/DoReMi-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app DoReMi-service:1.0-SNAPSHOT
    oc expose svc/DoReMi-service
else

	echo "Launching the application locally..."
	pattern="DoReMi-service"
	files=( $pattern )
	cd ${files[0]}
